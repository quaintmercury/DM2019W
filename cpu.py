...
class ALU(object):
    ...
    ALU_OPS = {
        ...
        OpCode.SUB: lambda x, y: x - y,     # Subtraction
        OpCode.MUL: lambda x, y: x * y,     # Multiplication
    ...
    def step(self):
        ...
        if compare > 0:
            ...
            if instr.op is OpCode.STORE:
                self.memory.put(resulting, self.registers[instr.reg_target].get())
            elif instr.op is OpCode.LOAD:
                self.registers[instr.reg_target].put(self.memory.get(resulting))
